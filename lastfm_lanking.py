...
def make_text(data: str, KIKAN: str, max_char_len: int) -> str:
    ...
    for i in range(len(result)):
        ...
        pre_ranking = f"{i + 1}. [{playcount}回] {name} \n\t🎤{artist}\n"
...
def text_length_fix(data: dict, kikan: str, char_len: int) -> str:
    """
    ツイートの文字数を500文字程度に調整
    """
...
def main():
    char_len = 48  # 曲名とアーティスト名の最大文字数を初期値として設定
    data = fetch_lastfm_json()
    logging.debug(f"Fetched data: {data}")
    text = text_length_fix(data, KIKAN, char_len)
    logging.info(f"Final text length: {len(text)} characters")
    print(text)
